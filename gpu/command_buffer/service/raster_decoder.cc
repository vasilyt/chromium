...
#undef RASTER_CMD_OP
  ...
  bool use_out_of_order_rasterization_ = false;
  ...
  std::unique_ptr<SharedImageRepresentationDeferred> shared_image_deferred_;
  base::Optional<SharedImageRepresentationDeferred::ScopedRecord>
      scoped_shared_image_record_;
  cc::PaintOpBuffer* paint_op_buffer_ = nullptr;
...
ContextResult RasterDecoderImpl::Initialize(
  ...
  if (attrib_helper.enable_oop_rasterization) {
    ...
    use_out_of_order_rasterization_ =
        gpu_preferences_.enable_oop_rasterization_ooor;
  }
...
void RasterDecoderImpl::DoBeginRasterCHROMIUM(GLuint sk_color,
  ...
  if (sk_surface_ || paint_op_buffer_) {
    LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION, "glBeginRasterCHROMIUM",
  ...
  DCHECK(!shared_image_ && !shared_image_deferred_);
  ...
  // Try using out of order rasterization.
  if (use_out_of_order_rasterization_) {
    shared_image_deferred_ =
        shared_image_representation_factory_.ProduceDeferred(mailbox);
  }

  if (shared_image_deferred_) {
    bool discard_previous_recording = !has_previous_content;
    scoped_shared_image_record_.emplace(shared_image_deferred_.get(), sk_color,
                                        msaa_sample_count, surface_props,
                                        discard_previous_recording);
    if (!scoped_shared_image_record_->success()) {
      shared_image_deferred_ = nullptr;
      scoped_shared_image_record_.reset();
      return;
    }
    paint_op_buffer_ = scoped_shared_image_record_->recorder();
    return;
  }
...
void RasterDecoderImpl::DoRasterCHROMIUM(GLuint raster_shm_id,
  ...
  if (paint_op_buffer_) {
    paint_op_buffer_->Deserialize(paint_buffer_memory, raster_shm_size,
                                  options);
    return;
  }
...
void RasterDecoderImpl::DoEndRasterCHROMIUM() {
  ...
  if (paint_op_buffer_) {
    DCHECK_GT(paint_op_buffer_->size(), 0u);
    scoped_shared_image_record_->set_release_callback(base::BindOnce(
...

    {
      if (use_ddl_) {
        TRACE_EVENT0(
            "gpu", "RasterDecoderImpl::DoEndRasterCHROMIUM::DetachAndDrawDDL");
